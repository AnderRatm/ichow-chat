	...
	public class AbstractSearchManager
	{
		...
		private function _populateServices(force:Boolean = false):void
		{
			if (force)
 			...
 			if (!_populated)
				new Browser(SparkManager.connectionManager.connection).getServiceItems(new EscapedJID(server), _handlePopulateServices);
		}
		...
		public function _handlePopulateServices(iq:IQ):void
		{
			...
			for each(var item:Object in itemExt.items) {
				...
				new Browser(SparkManager.connectionManager.connection).getServiceInfo(new EscapedJID(item.jid), _handlePopulateServicesFromInfo);
			}
		}
		...
		public function performSearch(jid:UnescapedJID, query:String, callback:Callback, fields:Array = null):void
		{
			...
			var iqSearch:IQ = new IQ(jid.escaped, IQ.TYPE_SET, null, accept);
